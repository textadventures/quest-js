...
namespace TextAdventures.Quest
{
    partial class GameLoader
    {
        ...
        private void AddExtendedAttributeLoaders()
        {
            ...
            {
                AddExtendedAttributeLoader((IExtendedAttributeLoader)Activator.CreateInstance(t));
            }
        }

        private void AddExtendedAttributeLoader(IExtendedAttributeLoader loader)
        {
        ...
        private class ScriptDictionaryLoader : ExtendedAttributeLoaderBase, IValueLoader
        {
            ...
            public override void Load(XmlReader reader, Element current)
            {
                ...
                current.Fields.Set(currentXmlElementName, ConvertToScriptDictionary(result));
            }
            ...
            private QuestDictionary<IScript> ConvertToScriptDictionary(IDictionary<string, string> dictionary)
            {
                ...
                {
                    IScript newScript = GameLoader.ScriptFactory.CreateScript(item.Value);
            ...
            public object GetValue(XElement xml)
            {
                ...
                xmlReader.Read();
                return ConvertToScriptDictionary(LoadScriptDictionary(xmlReader, null, "value"));
            }
        }
        ...
        private class ObjectDictionaryLoader : DictionaryLoaderBase<string>, IValueLoader
        {
            ...
            {
                var result = LoadDictionary(reader, current.Name);
                current.Fields.AddObjectDictionary(reader.Name, result);
            }

            public object GetValue(XElement xml)
            {
                // TODO - think we just need to copy over Lazy types from main Quest project here, and then Resolve at some point
